...
\node[rectangle split, rectangle split parts=8, rectangle split horizontal=true, draw] at (5, -1) {
$\mathsf{r: Nil}$
\nodepart{two}$\mathsf{s: Nil}$
\nodepart{three}$\mathsf{t: Nil}$
\nodepart{four}$\mathsf{u: Nil}$
\nodepart{five}$\mathsf{v: Nil}$
\nodepart{six}$\mathsf{w: Nil}$
\nodepart{seven}$\mathsf{x: Nil}$
\nodepart{eight}$\mathsf{y: Nil}$
};
\end{tikzpicture}
\end{center}
\end{frame}
...
$\mathsf{r: s}$
\nodepart{two}$\mathsf{s: Nil}$
\nodepart{three}$\mathsf{t: w}$
\nodepart{four}$\mathsf{u: Nil}$
\nodepart{five}$\mathsf{v: Nil}$
\nodepart{six}$\mathsf{w: s}$
\nodepart{seven}$\mathsf{x: w}$
\nodepart{eight}$\mathsf{y: Nil}$
};
\end{tikzpicture}
\end{center}
...
Na floresta de profundidade de um grafo, dirigido ou não, o vértice $v$ é
descendente de $u$ se somente se na etapa $\attrib{u}{d}$, $v$ pode ser
alcançado a partir de $u$ por um caminho composto apenas por vértices brancos.
\end{theorem}
}
\pause
\begin{proof}
...
$\Leftarrow$
\begin{itemize}
\item na etapa $\attrib{u}{d}$, há um caminho brancho de $u$ até $v$
\item hipótese 1: $v$ não é descendente de $u$ na árvore em profundidade
\item hipótese 2: os demais vértices no caminho são descendentes na árvore em profundidade
\item seja $w$ o predecessor de $v$, logo $\attrib{w}{f} < \attrib{u}{f}$
\item $v$ é processado após $u$, e antes que $w$ seja finalizado
\item logo $\attrib{u}{d} < \attrib{v}{d} < \attrib{w}{f} < \attrib{u}{f}$
\item pelo teorema dos parênteses, o intervalo para $v$ é contido no intervalo para $u$
...
\item arestas para frente (F): são as arestas processadas em $\proc{DFS-visit}$ que conectam $v$ a um descendente de $v$ já visitado \only<5>{\alert{$\attrib{v}{color} = \const{Black}$}} \only<5-6>{\alert{$\attrib{u}{d} < \attrib{v}{d}$}}
\item arestas cruzadas (C): as demais arestas. \only<5-6>{\alert{$\attrib{v}{color} = \const{Black}$}} \only<6>{\alert{$\attrib{u}{d} > \attrib{v}{d}$}}
\begin{itemize}
