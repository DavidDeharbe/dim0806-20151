...
\item pelo corolário do aninhamento, $\attrib{d}{u} < \attrib{d}{w}$.
\item logo, na etapa $\attrib{d}{u}$, $w$ é branco.
\end{itemize}
}
\only<3>{
$\Leftarrow$
\begin{itemize}
\item na etapa $\attrib{d}{u}$, $v$ é alcançável por um caminho branco a partir de $u$
...
\item arestas para frente: são as arestas processadas em $\proc{DFS-visit}$ que conectam $v$ a um descendente de $v$ já visitado \only<5>{\alert{$\attrib{v}{color} = \const{Black}$}} \only<5-6>{\alert{$\attrib{d}{u} < \attrib{d}{v}$}}
\item arestas cruzadas: as demais arestas. \only<5-6>{\alert{$\attrib{v}{color} = \const{Black}$}} \only<6>{\alert{$\attrib{d}{u} > \attrib{d}{v}$}}
\begin{itemize}
